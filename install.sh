...
runin() {
	{ "$@" 2>>../xilla-install.log || return $?; } | while read -r line; do
		printf "%s\n" "$line" >>../xilla-install.log
	done
}
...
if [ ! x"$SUDO_USER" = x"" ]; then
	chown "$SUDO_USER:" xilla-install.log
fi

if [ -d "Xilla/xilla" ]; then
	cd Xilla || {
...
echo "Installing..." >xilla-install.log
...
if echo "$OSTYPE" | grep -qE '^linux-gnu.*' && [ -f '/etc/debian_version' ]; then
	...
	printf "\r\033[1;31mUnrecognised OS.\e[0m Please follow 'Manual installation' at \033[0;94mhttps://github.com/xeroxdeveloper/Xilla/#-installation\e[0m"
	exit 1
fi
...
${SUDO_CMD}rm -rf Xilla
runout ${SUDO_CMD}git clone https://github.com/xeroxdeveloper/Xilla/ || {
