...
from xillatl.tl.functions.messages import (
...
class XillaSettingsMod(loader.Module):
        ...
        async for dialog in self._client.iter_dialogs(
            ...
            if (
                ...
                and (
                    ...
                    and dialog.name in {"xilla-logs", "silent-tags"}
                )
                or (
    ...
    async def inline__setting(self, call: InlineCall, key: str, state: bool = False):
        if callable(key):
            key()
            xillatl.extensions.html.CUSTOM_EMOJIS = not main.get_config_key(
    ...
    async def weburl(self, message: Message, force: bool = False):
        if "LAVHOST" in os.environ:
            form = await self.inline.form(
                ...
                reply_markup={
                    ...
                    "url": await main.xilla.web.get_url(proxy_pass=False),
                },
        ...
        if not main.xilla.web:
            main.xilla.web = core.Web(
                data_root=main.BASE_DIR,
                api_token=main.xilla.api_token,
                proxy=main.xilla.proxy,
                connection=main.xilla.conn,
            )
            await main.xilla.web.add_loader(self._client, self.allmodules, self._db)
            await main.xilla.web.start_if_ready(
                len(self.allclients),
                main.xilla.arguments.port,
                proxy_pass=main.xilla.arguments.proxy_pass,
            )
        if force:
            form = message
            await form.edit(
    ...
    async def invokecmd(self, message: Message):
        ...
        if module == "core":
            if method == "flush_entity_cache":
                result = (
                    f"Dropped {len(self._client._xilla_entity_cache)} cache records"
                )
                self._client._xilla_entity_cache = {}
            elif method == "flush_fulluser_cache":
                result = (
                    f"Dropped {len(self._client._xilla_fulluser_cache)} cache records"
                )
                self._client._xilla_fulluser_cache = {}
            elif method == "flush_fullchannel_cache":
                result = (
                    f"Dropped {len(self._client._xilla_fullchannel_cache)} cache"
                    " records"
                )
                ...
                result = (
                    f"Dropped {len(self._client._xilla_entity_cache)} entity cache"
                    " records\nDropped"
                    f" {len(self._client._xilla_fulluser_cache)} fulluser cache"
                    " records\nDropped"
                    f" {len(self._client._xilla_fullchannel_cache)} fullchannel cache"
                    " records\nDropped"
                ...
                result = (
                    "Entity cache:"
                    f" {len(self._client._xilla_entity_cache)} records\nFulluser cache:"
                    f" {len(self._client._xilla_fulluser_cache)} records\nFullchannel"
                    " cache:"
                    f" {len(self._client._xilla_fullchannel_cache)} records\nLoader"
