...
class XillaBackupMod(loader.Module):
    ...
    async def client_ready(self):
        if not self.get('period'):
            await self.inline.bot.send_photo(self.tg_id, photo='https://github.com/xeroxdeveloper/assets/raw/master/unit_alpha.png', caption=self.strings('period'), reply_markup=self.inline.generate_markup(utils.chunks([{'text': f'🕰 {i} h', 'callback': self._set_backup_period, 'args': (i,)} for i in [1, 2, 4, 6, 8, 12, 24, 48, 168]], 3) + [[{'text': '🚫 Never', 'callback': self._set_backup_period, 'args': (0,)}]]))
        self._backup_channel, _ = await utils.asset_channel(self._client, 'xilla-backups', '📼 Your database backups will appear here', silent=True, archive=True, avatar='https://github.com/xeroxdeveloper/assets/raw/master/xilla-backups.png', _folder='xilla')
    ...
    async def set_backup_period(self, message: Message):
        if not (args := utils.get_args_raw(message)) or not args.isdigit() or int(args) not in range(200):
    ...
    async def handler(self):
        try:
            ...
            if not self.get('last_backup'):
                ...
                return
            await asyncio.sleep(self.get('last_backup') + self.get('period') - time.time())
            backup = io.BytesIO(json.dumps(self._db).encode())
            backup.name = f'xilla-db-backup-{datetime.datetime.now():%d-%m-%Y-%H-%M}.json'
    ...
    async def backupdb(self, message: Message):
        ...
        await self._client.send_file('me', txt, caption=self.strings('backup_caption').format(prefix=utils.escape_html(self.get_prefix())))
    ...
    async def backupmods(self, message: Message):
        ...
        await utils.answer_file(message, archive, caption=self.strings('modules_backup').format(mods_quantity, utils.escape_html(self.get_prefix())))
    ...
    async def restoremods(self, message: Message):
        ...
        try:
            ...
            try:
                ...
                with zipfile.ZipFile(file) as zf:
                    with zf.open('db_mods.json', 'r') as modules:
                        ...
                        if isinstance(db_mods, dict) and all((isinstance(key, str) and isinstance(value, str) and utils.check_url(value) for key, value in db_mods.items())):
            ...
            if not isinstance(decoded_text, dict) or not all((isinstance(key, str) and isinstance(value, str) for key, value in decoded_text.items())):
