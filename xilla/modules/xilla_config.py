...
class XillaConfigMod(loader.Module):
        ...
        await utils.answer(
            ...
            reply_markup=[
                ...
                [
                    {
                        "text": "📦 Пакеты (Main)",
                        "callback": self.inline__packages_hub,
                    }
                ],
                [{"text": self.strings("close_btn"), "action": "close"}],
            ],
    ...
    async def inline__packages_hub(self, call: InlineCall, page: int = 0):
        kb = [
            [{"text": "📦 Main", "callback": self.inline__configure_package, "args": ("Main",)}],
            [
                {"text": self.strings("back_btn"), "callback": self.inline__choose_category},
                {"text": self.strings("close_btn"), "action": "close"}
            ]
        ]
        
        await call.edit(
            "<b>📦 Управление Пакетами</b>\n\nПакеты — это сложные надстройки над Xilla. Выберите пакет для настройки:",
            reply_markup=kb
        )

    async def inline__configure_package(self, call: InlineCall, package: str):
        if package == "Main":
            await call.edit(
                "<b>📦 Пакет Main</b>\n\nЭтот пакет добавляет возможность управлять другими пакетами, создавать и скачивать их.\n<i>Настройки пакета пока недоступны.</i>",
                reply_markup=[[
                    {"text": "◀️ Назад", "callback": self.inline__packages_hub},
                ]]
            )
            return
        await call.answer("Не реализовано", show_alert=True)
