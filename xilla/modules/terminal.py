...
class MessageEditor:
    def __init__(
        self,
        message: xillatl.tl.types.Message,
        command: str,
        config,
        strings,
        request_message,
    ...
    async def update_stderr(self, stderr):
        ...
        if lastlines[0] == self.PASS_REQ and self.state == 0:
            ...
            self.message[0].client.add_event_handler(
                ...
                xillatl.events.messageedited.MessageEdited(chats=["me"]),
            )
...
class RawMessageEditor(SudoMessageEditor):
    ...
    async def redraw(self):
        ...
        with contextlib.suppress(
            xillatl.errors.rpcerrorlist.MessageNotModifiedError,
            xillatl.errors.rpcerrorlist.MessageEmptyError,
            ValueError,
        ):
            try:
                await utils.answer(self.message, text)
