...
class SudoMessageEditor(MessageEditor):
    ...
    WRONG_PASS = '\\[sudo\\] password for (.*): Sorry, try again\\.'
    TOO_MANY_TRIES = '\\[sudo\\] password for (.*): sudo: [0-9]+ incorrect password attempts'
...
class TerminalMod(loader.Module):
    ...
    def __init__(self):
        self.config = loader.ModuleConfig(loader.ConfigValue('FLOOD_WAIT_PROTECT', 2, lambda: self.strings('fw_protect'), validator=loader.validators.Integer(minimum=0)))
        self.activecmds = {}
