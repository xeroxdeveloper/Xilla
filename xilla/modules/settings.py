...
class CoreMod(loader.Module):
    ...
    async def xillacmd(self, message: Message):
        await utils.answer_file(
            message,
            "https://github.com/xeroxdeveloper/assets/raw/master/xilla_cat_banner.mp4",
            self.strings("xilla").format(
                (
                    utils.get_platform_emoji()
                    if self._client.xilla_me.premium and CUSTOM_EMOJIS
                    else "🌘 <b>Xilla userbot</b>"
                ),
                *version.__version__,
                utils.get_commit_url(),
                f"{xillatl.__version__} #{xillatl.tl.alltlobjects.LAYER}",
                (
                    "<emoji document_id=5377399247589088543>🔥</emoji>"
                    if self._client.pyro_proxy
                    else "<emoji document_id=5418308381586759720>📴</emoji>"
                ),
                f"{xillapyro.__version__} #{xillapyro.raw.all.layer}",
            )
            + (
                ""
