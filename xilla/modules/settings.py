...
class CoreMod(loader.Module):
    ...
    async def unblacklist(self, message: Message):
        chatid = await self.blacklistcommon(message)
        self._db.set(main.__name__, 'blacklist_chats', list(set(self._db.get(main.__name__, 'blacklist_chats', [])) - {chatid}))
    ...
    async def unblacklistuser(self, message: Message):
        if not (user := (await self.getuser(message))):
            ...
            return
        self._db.set(main.__name__, 'blacklist_users', list(set(self._db.get(main.__name__, 'blacklist_users', [])) - {user}))
    ...
    async def setprefix(self, message: Message):
        ...
        if args == 's':
            await utils.answer(message, self.strings('prefix_incorrect'))
            return
        if not is_dragon and args[0] == self._db.get('dragon.prefix', 'command_prefix', ',') or (is_dragon and args[0] == self._db.get(main.__name__, 'command_prefix', '.')):
            await utils.answer(message, self.strings('prefix_collision'))
            return
        oldprefix = f'dragon {utils.escape_html(self.get_prefix('dragon'))}' if is_dragon else utils.escape_html(self.get_prefix())
        self._db.set('dragon.prefix' if is_dragon else main.__name__, 'command_prefix', args)
        await utils.answer(message, self.strings('prefix_set').format(DRAGON_EMOJI if is_dragon else '<emoji document_id=5197474765387864959>👍</emoji>', newprefix=utils.escape_html(self.get_prefix() if is_dragon else args[0]), oldprefix=utils.escape_html(oldprefix)))

    @loader.command()
    async def aliases(self, message: Message):
        await utils.answer(message, self.strings('aliases') + '\n'.join([f'▫️ <code>{i}</code> &lt;- {y}' for i, y in self.allmodules.aliases.items()]))
    ...
    async def cleardb(self, message: Message):
        await self.inline.form(self.strings('confirm_cleardb'), message, reply_markup=[{'text': self.strings('cleardb_confirm'), 'callback': self._inline__cleardb}, {'text': self.strings('cancel'), 'action': 'close'}])
