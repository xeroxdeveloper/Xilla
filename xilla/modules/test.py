...
class TestMod(loader.Module):
    ...
    async def logs(
        ...
        other = (
            *main.__version__,
            (
                " <a"
                f' href="https://github.com/xeroxdeveloper/Xilla/commit/{ghash}">@{ghash[:8]}</a>'
                if ghash
                else ""
            ),
        )
    ...
    async def ping(self, message: Message):
        ...
        msg = await utils.answer(message, "⚡ <b>Пинг...</b>")
        ping_ms = round((time.perf_counter_ns() - start) / 10**6, 2)
        
        up = utils.formatted_uptime()
        
        status = "🔴" if ping_ms > 500 else "🟡" if ping_ms > 200 else "🟢"
        
        text = (
            f"<b>{status} Pong!</b>\n"
            f"<b>⚡ Пинг:</b> <code>{ping_ms} ms</code>\n"
            f"<b>⏳ Аптайм:</b> <code>{up}</code>\n"
            f"<b>⚙️ Xilla v{'.'.join(map(str, main.__version__))}</b>"
        )
    ...
    async def client_ready(self):
        chat, _ = await utils.asset_channel(
            self._client,
            "xilla-logs",
            "☀️ Your Xilla logs will appear in this chat",
            silent=True,
            invite_bot=True,
            avatar="https://github.com/xeroxdeveloper/assets/raw/master/xilla-logs.png",
        )
