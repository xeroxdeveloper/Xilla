...
CONSTRUCTORS = {(lambda x: x[0].lower() + x[1:])(method.__class__.__name__.rsplit('Request', 1)[0]): method.CONSTRUCTOR_ID for method in utils.array_sum([[method for method in dir(getattr(functions, group)) if isinstance(method, TLRequest)] for group in GROUPS])}
...
class APIRatelimiterMod(loader.Module):
    ...
    async def _install_protection(self):
        ...
        async def new_call(sender: 'MTProtoSender', request: TLRequest, ordered: bool=False, flood_sleep_threshold: int=None):
            await asyncio.sleep(random.randint(1, 5) / 100)
            req = (request,) if not is_list_like(request) else request
            for r in req:
                if time.perf_counter() > self._suspend_until and (not self.get('disable_protection', True)) and (r.__module__.rsplit('.', maxsplit=1)[1] in {'messages', 'account', 'channels'}):
                    request_name = type(r).__name__
                    self._ratelimiter += [(request_name, time.perf_counter())]
                    self._ratelimiter = list(filter(lambda x: time.perf_counter() - x[1] < int(self.config['time_sample']), self._ratelimiter))
                    if len(self._ratelimiter) > int(self.config['threshold']) and (not self._lock):
                        self._lock = True
                        report = io.BytesIO(json.dumps(self._ratelimiter, indent=4).encode())
                        report.name = 'local_fw_report.json'
                        await self.inline.bot.send_document(self.tg_id, report, caption=self.inline.sanitise_text(self.strings('warning').format(self.config['local_floodwait'], prefix=utils.escape_html(self.get_prefix()))))
    ...
    async def api_fw_protection(self, message: Message):
        await self.inline.form(message=message, text=self.strings('u_sure'), reply_markup=[{'text': self.strings('btn_no'), 'action': 'close'}, {'text': self.strings('btn_yes'), 'callback': self._finish}])
    ...
    async def debugger(self, message: Message):
        if not self._debugger:
            ...
            return
        await self.inline.form(message=message, text=self.strings('web_pin'), reply_markup=[[{'text': self.strings('web_pin_btn'), 'callback': self._show_pin}], [{'text': self.strings('proxied_url'), 'url': self._debugger.url}, {'text': self.strings('local_url'), 'url': f'http://127.0.0.1:{self._debugger.port}'}]])
