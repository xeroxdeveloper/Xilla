...
SUPPORTED_LANGUAGES = {'en': '🇬🇧 English', 'ru': '🇷🇺 Русский', 'fr': '🇫🇷 Français', 'it': '🇮🇹 Italiano', 'de': '🇩🇪 Deutsch', 'tr': '🇹🇷 Türkçe', 'uz': "🇺🇿 O'zbekcha", 'es': '🇪🇸 Español', 'kk': '🇰🇿 Қазақша', 'tt': '🥟 Татарча'}
...
class BaseTranslator:
    ...
    def _get_pack_content(self, pack: Path, prefix: str='xilla.modules.') -> typing.Optional[dict]:
        return self._get_pack_raw(pack.read_text(), pack.suffix, prefix)

    def _get_pack_raw(self, content: str, suffix: str, prefix: str='xilla.modules.') -> typing.Optional[dict]:
        if suffix == '.json':
            return json.loads(content)
        content = yaml.load(content)
        if all((len(key) == 2 for key in content)):
            return {language: {{f'{module.strip('$')}.{key}' if module.startswith('$') else f'{prefix}{module}.{key}': value for module, strings in pack.items() for key, value in strings.items() if key != 'name'}} for language, pack in content.items()}
        return {f'{module.strip('$')}.{key}' if module.startswith('$') else f'{prefix}{module}.{key}': value for module, strings in content.items() for key, value in strings.items() if key != 'name'}
    ...
    async def load_module_translations(self, pack_url: str) -> typing.Union[bool, dict]:
        ...
        if (lang := self.db.get(__name__, 'lang', False)):
            return next((data[language] for language in lang.split() if language in data), data.get('en', {}))
        return data.get('en', {})
        ...
        if (lang := self.db.get(__name__, 'lang', False)):
            for language in lang.split():
                if utils.check_url(language):
                    try:
                        data = self._get_pack_raw((await utils.run_sync(requests.get, language)).text, language.split('.')[-1])
                    except Exception:
                    ...
                    any_ = True
                    continue
                for possible_path in [PACKS / f'{language}.json', PACKS / f'{language}.yml']:
...
class Strings:
    ...
    def __getitem__(self, key: str) -> str:
        return self.external_strings.get(key, None) or (self._translator.getkey(f'{self._mod.__module__}.{key}') if self._translator is not None else False) or (getattr(self._mod, next((f'strings_{lang}' for lang in self._translator.db.get(__name__, 'lang', 'en').split(' ') if hasattr(self._mod, f'strings_{lang}') and isinstance(getattr(self._mod, f'strings_{lang}'), dict) and (key in getattr(self._mod, f'strings_{lang}'))), utils.rand(32)), self._base_strings) if self._translator is not None else self._base_strings).get(key, self._base_strings.get(key, 'Unknown strings'))
