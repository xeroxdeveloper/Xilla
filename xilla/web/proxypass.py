...
class ProxyPasser:
    ...
    async def _read_stream(self, callback: callable, stream: typing.BinaryIO, delay: int) -> None:
    ...
    async def get_url(self, port: int, no_retry: bool=False) -> typing.Optional[str]:
        async with self._lock:
            ...
            self._sproc = await asyncio.create_subprocess_shell(f'ssh -o StrictHostKeyChecking=no -R 80:127.0.0.1:{port} nokey@localhost.run', stdin=asyncio.subprocess.PIPE, stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE)
            utils.atexit(self.kill)
            self._url_available = asyncio.Event()
            logger.debug('Starting proxy pass reader for port %d', port)
            asyncio.ensure_future(self._read_stream(self._process_stream, self._sproc.stdout, 1))
            try:
