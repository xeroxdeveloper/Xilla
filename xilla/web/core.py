    ...
    async def get_url(self, proxy_pass: bool) -> str:
        ...
        if proxy_pass:
            with contextlib.suppress(Exception):
                url = await asyncio.wait_for(self.proxypasser.get_url(self.port), timeout=10)
        if not url:
            ip = '127.0.0.1' if 'DOCKER' not in os.environ else subprocess.run(['hostname', '-i'], stdout=subprocess.PIPE, check=True).stdout.decode('utf-8').strip()
