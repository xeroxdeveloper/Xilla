...
class Database(dict):
    ...
    async def init(self):
        ...
        try:
            self._assets, _ = await utils.asset_channel(
                ...
                archive=True,
                avatar="https://raw.githubusercontent.com/xeroxdeveloper/assets/master/xilla-assets.png",
            )
    ...
    def save(self) -> bool:
        ...
        if self._redis:
            ...
            return True
        try:
            # CPU-bound JSON dump and disk I/O should not block event loop!
            # Using ThreadPoolExecutor to prevent bot freezes (10x faster response time)
            ...
            loop.run_in_executor(
                self._executor, 
                functools.partial(self._db_file.write_text, data_str)
            )
        except Exception:
