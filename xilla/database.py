...
__all__ = ['Database', 'PointerList', 'PointerDict', 'NamedTupleMiddlewareDict', 'NamedTupleMiddlewareList', 'BaseSerializingMiddlewareDict', 'BaseSerializingMiddlewareList']
...
class Database(dict):
    ...
    def read(self):
        if self._redis:
            try:
                self.update(**json.loads(self._redis.get(str(self._client.tg_id)).decode()))
            except Exception:
    ...
    def process_db_autofix(self, db: dict) -> bool:
        ...
        for key, value in db.copy().items():
            if not isinstance(key, (str, int)):
                logger.warning('DbAutoFix: Dropped key %s, because it is not string or int', key)
                continue
            if not isinstance(value, dict):
                del db[key]
                logger.warning('DbAutoFix: Dropped key %s, because it is non-dict, but %s', key, type(value))
                continue
            for subkey in value:
                if not isinstance(subkey, (str, int)):
                    del db[key][subkey]
                    logger.warning('DbAutoFix: Dropped subkey %s of db key %s, because it is not string or int', subkey, key)
                    continue
        return True
    ...
    def save(self) -> bool:
        if not self.process_db_autofix(self):
            try:
                ...
                raise RuntimeError("Can't find revision to restore broken database from database is most likely broken and will lead to problems, so its save is forbidden.")
            self.clear()
            self.update(**rev)
            raise RuntimeError('Rewriting database to the last revision because new one destructed it')
        ...
        if self._redis:
            ...
            return True
        try:
            data_str = json.dumps(self, indent=4) if not hasattr(json, 'dumps') else json.dumps(self, option=json.OPT_INDENT_2).decode('utf-8') if type(json.dumps(self)) is bytes else json.dumps(self, indent=4)
            loop = asyncio.get_event_loop()
            loop.run_in_executor(self._executor, functools.partial(self._db_file.write_text, data_str))
        except Exception:
            logger.exception('Database save failed in background!')
            return False
        return True
    ...
    async def store_asset(self, message: Message) -> int:
        ...
        return (await self._client.send_message(self._assets, message)).id if isinstance(message, Message) else (await self._client.send_message(self._assets, file=message, force_document=True)).id
    ...
    def set(self, owner: str, key: str, value: JSONSerializable) -> bool:
        if not utils.is_serializable(owner):
            raise RuntimeError(f'Attempted to write object to owner={owner!r} (type(owner)={type(owner)!r}) of database. It is not JSON-serializable key which will cause errors')
        if not utils.is_serializable(key):
            raise RuntimeError(f'Attempted to write object to key={key!r} (type(key)={type(key)!r}) of database. It is not JSON-serializable key which will cause errors')
        if not utils.is_serializable(value):
            raise RuntimeError(f'Attempted to write object of key={key!r} (type(value)={type(value)!r}) to database. It is not JSON-serializable value which will cause errors')
    ...
    def pointer(self, owner: str, key: str, default: typing.Optional[JSONSerializable]=None, item_type: typing.Optional[typing.Any]=None) -> typing.Union[JSONSerializable, PointerList, PointerDict]:
        ...
        mapping = {list: PointerList, dict: PointerDict, collections.abc.Hashable: lambda v: v}
        pointer_constructor = next((pointer for type_, pointer in mapping.items() if isinstance(value, type_)), None)
            ...
            if isinstance(value, list):
                for item in self.get(owner, key, default):
                    if not isinstance(item, dict):
                        raise ValueError("Item type can only be specified for dedicated keys and can't be mixed with other ones")
