...
class CommandDispatcher:
    def __init__(
        ...
        self._cached_usernames = [
            (
                self._client.xilla_me.username.lower()
                if self._client.xilla_me.username
                else str(self._client.xilla_me.id)
            )
        ]
        self._cached_usernames.extend(
            getattr(self._client.xilla_me, "usernames", None) or []
        )
        self.raw_handlers = []
    ...
    async def _handle_command(
        ...
        if (
            ...
            and message.chat.title.startswith("xilla-")
            and message.chat.title != "xilla-logs"
        ):
            if not watcher:
