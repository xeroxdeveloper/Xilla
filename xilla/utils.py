...
from xillatl.tl.functions.channels import (
...
def get_chat_id(message: typing.Union[Message, AiogramMessage]) -> int:
    ...
    :return: Chat ID
    """
    return xillatl.utils.resolve_id(
...
def run_sync(func, *args, **kwargs):
    """
    Run a non-async function in a new thread pool to prevent event loop blocking.
    Max workers set to 32 to handle heavy IO operations across modules.
    """
    ...
    **kwargs,
    ...
    :example:
        ...
        >>> await utils.answer_file(
            message,
            "https://mods.xeroxdeveloper.ru/badges/artai.jpg",
...
async def answer(
    ...
    response: str,
    *,
    reply_markup: typing.Optional[XillaReplyMarkup] = None,
    **kwargs,
    ...
    if not (edit := (message.out and not message.via_bot_id and not message.fwd_from)):
        ...
        kwargs.pop("reply_to")
    parse_mode = xillatl.utils.sanitize_parse_mode(
        kwargs.pop(
            "parse_mode",
...
def get_commit_url() -> str:
    ...
    try:
        hash_ = get_git_hash()
        return (
            f'<a href="https://github.com/xeroxdeveloper/Xilla/commit/{hash_}">#{hash_[:7]}</a>'
        )
    except Exception:
        return "Unknown"
...
def get_git_info() -> typing.Tuple[str, str]:
    ...
    return (
        hash_,
        f"https://github.com/xeroxdeveloper/Xilla/commit/{hash_}" if hash_ else "",
    )
