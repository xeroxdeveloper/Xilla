...
PM = 1 << 12
EVERYONE = 1 << 13
BITMAP = {'OWNER': OWNER, 'GROUP_OWNER': GROUP_OWNER, 'GROUP_ADMIN_ADD_ADMINS': GROUP_ADMIN_ADD_ADMINS, 'GROUP_ADMIN_CHANGE_INFO': GROUP_ADMIN_CHANGE_INFO, 'GROUP_ADMIN_BAN_USERS': GROUP_ADMIN_BAN_USERS, 'GROUP_ADMIN_DELETE_MESSAGES': GROUP_ADMIN_DELETE_MESSAGES, 'GROUP_ADMIN_PIN_MESSAGES': GROUP_ADMIN_PIN_MESSAGES, 'GROUP_ADMIN_INVITE_USERS': GROUP_ADMIN_INVITE_USERS, 'GROUP_ADMIN': GROUP_ADMIN, 'GROUP_MEMBER': GROUP_MEMBER, 'PM': PM, 'EVERYONE': EVERYONE}
GROUP_ADMIN_ANY = GROUP_ADMIN_ADD_ADMINS | GROUP_ADMIN_CHANGE_INFO | GROUP_ADMIN_BAN_USERS | GROUP_ADMIN_DELETE_MESSAGES | GROUP_ADMIN_PIN_MESSAGES | GROUP_ADMIN_INVITE_USERS | GROUP_ADMIN
...
class SecurityManager:
    ...
    def add_rule(self, target_type: str, target: EntityLike, rule: str, duration: int):
        if target_type not in {'chat', 'user'}:
            raise ValueError(f'Invalid target_type: {target_type}')
        if all((not rule.startswith(rule_type) for rule_type in {'command', 'module', 'inline'})):
            raise ValueError(f'Invalid rule: {rule}')
        if duration < 0:
            raise ValueError(f'Invalid duration: {duration}')
        (self._tsec_chat if target_type == 'chat' else self._tsec_user).append({'target': target.id, 'rule_type': rule.split('/')[0], 'rule': rule.split('/', maxsplit=1)[1], 'expires': int(time.time() + duration) if duration else 0, 'entity_name': get_display_name(target), 'entity_url': utils.get_entity_url(target)})
    ...
    def get_flags(self, func: typing.Union[Command, int]) -> int:
        if isinstance(func, int):
            ...
            config = self._db.get(__name__, 'masks', {}).get(f'{func.__module__}.{func.__name__}', getattr(func, 'security', self._default))
    ...
    def _check_tsec_inline(self, user_id: int, command: str) -> bool:
        return command and any((rule['target'] == user_id and rule['rule_type'] == 'inline' and (rule['rule'] == command) for rule in self._tsec_user))
        ...
        for info in self._sgroups.copy().values():
            if user_id in info.users:
                for permission in info.permissions:
                    if permission['rule_type'] == 'command' and permission['rule'] == command or (permission['rule_type'] == 'module' and permission['rule'] == command):
                        return True
        for info in self._tsec_user.copy():
            if info['target'] == user_id and (info['rule_type'] == 'command' and info['rule'] == command or (info['rule_type'] == 'module' and command in self._client.loader.commands and (info['rule'] == self._client.loader.commands[command].__qualname__.split('.')[0]))):
                return True
        return False

    async def check(self, message: typing.Optional[Message], func: typing.Union[Command, int], user_id: typing.Optional[int]=None, inline_cmd: typing.Optional[str]=None, *, usernames: typing.Optional[typing.List[str]]=None) -> bool:
        self._reload_rights()
        ...
        is_channel = False
        if message and message.is_channel and (not message.is_group) and message.edit_date:
            async for event in self._client.iter_admin_log(utils.get_chat_id(message), limit=10, edit=True):
                if event.action.prev_message.id == message.id:
                    ...
                    is_channel = True
        if user_id == self._client.tg_id or (getattr(message, 'out', False) and (not is_channel)):
            return True
        ...
        if config & SUDO or config & SUPPORT:
            if not self._last_warning or time.time() - self._last_warning > 60 * 60:
                import warnings
                warnings.warn('You are using module containing SUDO or SUPPORT security groups, which are deprecated. It might behave strangely', DeprecationWarning)
        ...
        f_pm = config & PM
        f_group_admin_any = f_group_admin_add_admins or f_group_admin_change_info or f_group_admin_ban_users or f_group_admin_delete_messages or f_group_admin_pin_messages or f_group_admin_invite_users or f_group_admin
        if user_id in self._owner:
            return True
        if user_id in self._db.get(main.__name__, 'blacklist_users', []):
            return False
        if message is None:
            return self._check_tsec_inline(user_id, inline_cmd) or bool(config & EVERYONE)
        try:
        ...
        if callable(func):
            ...
            for info in self._sgroups.copy().values():
                if user_id in info.users:
                    for permission in info.permissions:
                        if permission['rule_type'] == 'command' and permission['rule'] == command:
                            logger.debug('sgroup match for %s', command)
                            return True
                        if permission['rule_type'] == 'module' and permission['rule'] == func.__self__.__class__.__name__:
                            logger.debug('sgroup match for %s', func.__self__.__class__.__name__)
                            return True
            for info in self._tsec_user.copy():
                if info['target'] == user_id:
                    if info['rule_type'] == 'command' and info['rule'] == command:
                        ...
                        return True
                    if info['rule_type'] == 'module' and info['rule'] == func.__self__.__class__.__name__:
                        logger.debug('tsec match for user %s', func.__self__.__class__.__name__)
                        return True
            if chat:
                for info in self._tsec_chat.copy():
                    if info['target'] == chat:
                        if info['rule_type'] == 'command' and info['rule'] == command:
                            ...
                            return True
                        if info['rule_type'] == 'module' and info['rule'] == func.__self__.__class__.__name__:
                            logger.debug('tsec match for %s', func.__self__.__class__.__name__)
                            return True
        ...
        if message.is_channel:
            if not message.is_group:
                ...
                if chat_id in self._cache and self._cache[chat_id]['exp'] >= time.time():
                    chat = self._cache[chat_id]['chat']
                else:
                    chat = await message.get_chat()
                    self._cache[chat_id] = {'chat': chat, 'exp': time.time() + 5 * 60}
                if not chat.creator and (not chat.admin_rights) or (not chat.creator and (not chat.admin_rights.post_messages)):
                    return False
                ...
                if cache_obj in self._cache and self._cache[cache_obj]['exp'] >= time.time():
                    participant = self._cache[cache_obj]['user']
                else:
                    participant = await message.client.get_permissions(message.peer_id, user_id)
                    self._cache[cache_obj] = {'user': participant, 'exp': time.time() + 5 * 60}
                if participant.is_creator or (participant.is_admin and (self._any_admin and f_group_admin_any or f_group_admin or (f_group_admin_add_admins and participant.add_admins) or (f_group_admin_change_info and participant.change_info) or (f_group_admin_ban_users and participant.ban_users) or (f_group_admin_delete_messages and participant.delete_messages) or (f_group_admin_pin_messages and participant.pin_messages) or (f_group_admin_invite_users and participant.invite_users))):
                    return True
            return False
        if message.is_group and (f_group_admin_any or f_group_owner):
            ...
            if cache_obj in self._cache and self._cache[cache_obj]['exp'] >= time.time():
                ...
                participant = next((possible_participant for possible_participant in participants if possible_participant.user_id == message.sender_id), None)
                self._cache[cache_obj] = {'user': participant, 'exp': time.time() + 5 * 60}
            if not participant:
                return
            if isinstance(participant, ChatParticipantCreator) or (isinstance(participant, ChatParticipantAdmin) and f_group_admin_any):
                return True
        return False
    _check = check