...
from .types import Command, ConfigValue, CoreOverwriteError, CoreUnloadError, DragonModule, InlineMessage, JSONSerializable, Library, LibraryConfig, LoadError, Module, ModuleConfig, SelfSuspend, SelfUnload, StopLoop, StringLoader, get_callback_handlers, get_commands, get_inline_handlers
__all__ = ['Modules', 'InfiniteLoop', 'Command', 'CoreOverwriteError', 'CoreUnloadError', 'DragonModule', 'InlineMessage', 'JSONSerializable', 'Library', 'LibraryConfig', 'LoadError', 'Module', 'SelfSuspend', 'SelfUnload', 'StopLoop', 'StringLoader', 'get_commands', 'get_inline_handlers', 'get_callback_handlers', 'validators', 'Database', 'InlineManager', 'Strings', 'Translator', 'ConfigValue', 'ModuleConfig', 'owner', 'group_owner', 'group_admin_add_admins', 'group_admin_change_info', 'group_admin_ban_users', 'group_admin_delete_messages', 'group_admin_pin_messages', 'group_admin_invite_users', 'group_admin', 'group_member', 'pm', 'unrestricted', 'inline_everyone', 'loop']
...
class InfiniteLoop:
    ...
    def __init__(self, func: FunctionType, interval: int, autostart: bool, wait_before: bool, stop_clause: typing.Union[str, None]):
        self.func = func
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _xilla_client_id_logging_tag = copy.copy(self.module_instance.allmodules.client.tg_id)
        if self._task:
    ...
    async def actual_loop(self, *args, **kwargs):
        ...
        while self.status:
            ...
            if isinstance(self._stop_clause, str) and self._stop_clause and (not self.module_instance.get(self._stop_clause, False)):
                break
            try:
...
def loop(interval: int=5, autostart: typing.Optional[bool]=False, wait_before: typing.Optional[bool]=False, stop_clause: typing.Optional[str]=None) -> FunctionType:
...
BASE_DIR = '/data' if 'DOCKER' in os.environ else os.path.normpath(os.path.join(utils.get_base_dir(), '..'))
    ...
    def config_complete(self, *args, **kwargs):
        ...
        def proccess_decorators(mark: str, obj: str):
            ...
            for attr in dir(func_):
                if attr.endswith('_doc') and len(attr) == 6 and isinstance(getattr(func_, attr), str):
        ...
        return self.config_complete._old_(self, *args, **kwargs) if not kwargs.pop('reload_dynamic_translate', None) else True
...
class Modules:
    ...
    def __init__(self, client: 'CustomTelegramClient', db: Database, allclients: list, translator: Translator):
    ...
    async def _junk_collector(self):
        while True:
            ...
            logger.debug('Reloaded %s commands, %s inline handlers, %s callback handlers and %s watchers', len(self.commands), len(self.inline_handlers), len(self.callback_handlers), len(self.watchers))
    ...
    async def _register_modules(self, modules: list, origin: str='<core>') -> typing.List[Module]:
        with contextlib.suppress(AttributeError):
        ...
        for name, obj in vars(module).items():
            for handler, group in getattr(obj, 'handlers', []):
                try:
                    ...
                    logging.exception("Can't add handler %s due to %s: %s", name, type(e).__name__, e)
    ...
    def unload_dragon(self, instance: DragonModule) -> bool:
        for handler in instance.handlers:
            try:
                ...
                logging.exception("Can't remove handler %s due to %s: %s", handler, type(e).__name__, e)
        if instance in self.dragon_modules:
            self.dragon_modules.remove(instance)
            return True
        return False

    async def register_module(self, spec: importlib.machinery.ModuleSpec, module_name: str, origin: str='<core>', save_fs: bool=False, is_dragon: bool=False) -> typing.Union[Module, typing.Tuple[ModuleType, DragonModule]]:
        with contextlib.suppress(AttributeError):
            _xilla_client_id_logging_tag = copy.copy(self.client.tg_id)
        ...
        ret = None
        ret = next((value() for value in vars(module).values() if inspect.isclass(value) and issubclass(value, Module)), None)
        ...
        if save_fs:
            path = os.path.join(LOADED_MODULES_DIR, f'{cls_name}_{self.client.tg_id}.py')
            if origin == '<string>':
    ...
    def register_raw_handlers(self, instance: Module):
        for name, handler in utils.iter_attrs(instance):
            if getattr(handler, 'is_raw_handler', False):
                ...
                logger.debug('Registered raw handler %s for %s. ID: %s', name, instance.__class__.__name__, handler.id)
    ...
    def register_commands(self, instance: Module):
        ...
        if instance.__origin__.startswith('<core'):
            self._core_commands += list(map(lambda x: x.lower(), list(instance.xilla_commands)))
        for _command, cmd in instance.xilla_commands.items():
            if not self._remove_core_protection and _command.lower() in self._core_commands and (not instance.__origin__.startswith('<core')):
        ...
        for alias, cmd in self.aliases.copy().items():
            if cmd in instance.xilla_commands:
                self.add_alias(alias, cmd)
    ...
    def register_inline_stuff(self, instance: Module):
        for name, func in instance.xilla_inline_handlers.copy().items():
            if name.lower() in self.inline_handlers:
                if hasattr(func, '__self__') and hasattr(self.inline_handlers[name], '__self__') and (func.__self__.__class__.__name__ != self.inline_handlers[name].__self__.__class__.__name__):
                    logger.debug('Duplicate inline_handler %s of %s', name, instance.__class__.__name__)
                logger.debug('Replacing inline_handler %s for %s', self.inline_handlers[name], instance.__class__.__name__)
            self.inline_handlers.update({name.lower(): func})
        for name, func in instance.xilla_callback_handlers.copy().items():
            if name.lower() in self.callback_handlers and (hasattr(func, '__self__') and hasattr(self.callback_handlers[name], '__self__') and (func.__self__.__class__.__name__ != self.callback_handlers[name].__self__.__class__.__name__)):
                logger.debug('Duplicate callback_handler %s of %s', name, instance.__class__.__name__)
            self.callback_handlers.update({name.lower(): func})

    def unregister_inline_stuff(self, instance: Module, purpose: str):
        for name, func in instance.xilla_inline_handlers.copy().items():
            if name.lower() in self.inline_handlers and (hasattr(func, '__self__') and hasattr(self.inline_handlers[name], '__self__') and (func.__self__.__class__.__name__ == self.inline_handlers[name].__self__.__class__.__name__)):
                del self.inline_handlers[name.lower()]
                logger.debug('Unregistered inline_handler %s of %s for %s', name, instance.__class__.__name__, purpose)
        for name, func in instance.xilla_callback_handlers.copy().items():
            if name.lower() in self.callback_handlers and (hasattr(func, '__self__') and hasattr(self.callback_handlers[name], '__self__') and (func.__self__.__class__.__name__ == self.callback_handlers[name].__self__.__class__.__name__)):
                del self.callback_handlers[name.lower()]
                logger.debug('Unregistered callback_handler %s of %s for %s', name, instance.__class__.__name__, purpose)
    ...
    def lookup(self, modname: str, include_dragon: bool=False) -> typing.Union[bool, Module, DragonModule, Library]:
        return next((lib for lib in self.libraries if lib.name.lower() == modname.lower()), False) or next((mod for mod in self.modules if mod.__class__.__name__.lower() == modname.lower() or mod.name.lower() == modname.lower()), False) or (next((mod for mod in self.dragon_modules if mod.name.lower() == modname.lower()), False) if include_dragon else False)
        ...
        for module in self.modules:
            if module.__class__.__name__ == instance.__class__.__name__:
                if not self._remove_core_protection and module.__origin__.startswith('<core'):
                    raise CoreOverwriteError(module=module.__class__.__name__[:-3] if module.__class__.__name__.endswith('Mod') else module.__class__.__name__)
                ...
                for _, method in utils.iter_attrs(module):
                    if isinstance(method, InfiniteLoop):
                        method.stop()
                        logger.debug('Stopped loop in module %s, method %s', module, method)
        self.modules += [instance]

    def find_alias(self, alias: str, include_legacy: bool=False) -> typing.Optional[str]:
        if not alias:
            return None
        for command_name, _command in self.commands.items():
            ...
            if not aliases:
                continue
            if any((alias.lower() == _alias.lower() and alias.lower() not in self._core_commands for _alias in aliases)):
                return command_name
    ...
    def dispatch(self, _command: str) -> typing.Tuple[str, typing.Optional[str]]:
        return next(((cmd, self.commands[cmd.lower()]) for cmd in [_command, self.aliases.get(_command.lower()), self.find_alias(_command)] if cmd and cmd.lower() in self.commands), (_command, None))
    ...
    def send_config_one(self, mod: Module, skip_hook: bool=False):
        ...
        if hasattr(mod, 'config'):
            modcfg = self._db.get(mod.__class__.__name__, '__config__', {})
            try:
                for conf in mod.config:
                    with contextlib.suppress(validators.ValidationError):
                        mod.config.set_no_raise(conf, modcfg[conf] if conf in modcfg else os.environ.get(f'{mod.__class__.__name__}.{conf}') or mod.config.getdef(conf))
            except AttributeError:
                logger.warning('Got invalid config instance. Expected `ModuleConfig`, got %s, %s', type(mod.config), mod.config)
    ...
    async def send_ready_one(self, mod: Module, no_self_unload: bool=False, from_dlmod: bool=False):
        with contextlib.suppress(AttributeError):
            _xilla_client_id_logging_tag = copy.copy(self.client.tg_id)
        if from_dlmod:
            try:
        ...
        try:
            ...
            logger.exception('Failed to send mod init complete signal for %s due to %s, attempting unload', mod, e)
            self.modules.remove(mod)
            raise
    ...
    def get_classname(self, name: str) -> str:
        return next((module.__class__.__module__ for module in reversed(self.modules) if name in (module.name, module.__class__.__module__)), name)
    ...
    async def unload_module(self, classname: str) -> typing.List[str]:
        ...
        for module in self.modules:
            if classname.lower() in (module.name.lower(), module.__class__.__name__.lower()):
                ...
                path = os.path.join(LOADED_MODULES_DIR, f'{name}_{self.client.tg_id}.py')
    ...
    def unregister_loops(self, instance: Module, purpose: str):
        for name, method in utils.iter_attrs(instance):
            if isinstance(method, InfiniteLoop):
                logger.debug('Stopping loop for %s in module %s, method %s', purpose, instance.__class__.__name__, name)
                method.stop()
    ...
    def unregister_commands(self, instance: Module, purpose: str):
        for name, cmd in self.commands.copy().items():
            if cmd.__self__.__class__.__name__ == instance.__class__.__name__:
                logger.debug('Removing command %s of module %s for %s', name, instance.__class__.__name__, purpose)
    ...
    def unregister_watchers(self, instance: Module, purpose: str):
        for _watcher in self.watchers.copy():
            if _watcher.__self__.__class__.__name__ == instance.__class__.__name__:
                logger.debug('Removing watcher %s of module %s for %s', _watcher, instance.__class__.__name__, purpose)
                self.watchers.remove(_watcher)
    ...
    def unregister_raw_handlers(self, instance: Module, purpose: str):
        for handler in self.client.dispatcher.raw_handlers:
            if handler.__self__.__class__.__name__ == instance.__class__.__name__:
                ...
                logger.debug('Unregistered raw handler of module %s for %s. ID: %s', instance.__class__.__name__, purpose, handler.id)
