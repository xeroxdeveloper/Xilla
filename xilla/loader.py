...
class InfiniteLoop:
    ...
    def stop(self, *args, **kwargs):
        with contextlib.suppress(AttributeError):
            _xilla_client_id_logging_tag = copy.copy(  
...
def tag(*tags, **kwarg_tags):
    ...
    @loader.tag("only_messages", "only_pm", regex=r"^[.] ?xilla$", from_id=0000000000)
...
class Modules:
    ...
    def get_prefix(self, userbot: typing.Optional[str] = None) -> str:
        """Get prefix for specific userbot. Pass `None` to get Xilla prefix"""
        if userbot == "dragon":
            key = "dragon.prefix"
            default = ","
        else:
