...
import grapheme
from aiogram.types import InlineQuery, InlineQueryResultArticle, InlineQueryResultAudio, InlineQueryResultDocument, InlineQueryResultGif, InlineQueryResultLocation, InlineQueryResultPhoto, InlineQueryResultVideo, InputTextMessageContent
...
class Form(InlineUnit):
    ...
    async def form(self, text: str, message: typing.Union[Message, int], reply_markup: typing.Optional[XillaReplyMarkup]=None, *, force_me: bool=False, always_allow: typing.Optional[typing.List[int]]=None, manual_security: bool=False, disable_security: bool=False, ttl: typing.Optional[int]=None, on_unload: typing.Optional[callable]=None, photo: typing.Optional[str]=None, gif: typing.Optional[str]=None, file: typing.Optional[str]=None, mime_type: typing.Optional[str]=None, video: typing.Optional[str]=None, location: typing.Optional[str]=None, audio: typing.Optional[typing.Union[dict, str]]=None, silent: bool=False) -> typing.Union[InlineMessage, bool]:
        ...
        if not isinstance(reply_markup, (list, dict)):
            logger.error('Invalid type for `reply_markup`. Expected `list` or `dict`, got `%s`', type(reply_markup))
            return False
        if photo and (not isinstance(photo, str) or not utils.check_url(photo)):
            logger.error('Invalid type for `photo`. Expected `str` with URL, got `%s`', type(photo))
            return False
        try:
        ...
        if gif and (not isinstance(gif, str) or not utils.check_url(gif)):
            logger.error('Invalid type for `gif`. Expected `str` with URL, got `%s`', type(gif))
            return False
        if file and (not isinstance(file, str) or not utils.check_url(file)):
            logger.error('Invalid type for `file`. Expected `str` with URL, got `%s`', type(file))
            return False
        ...
        if video and (not isinstance(video, str) or not utils.check_url(video)):
            logger.error('Invalid type for `video`. Expected `str` with URL, got `%s`', type(video))
            return False
        if isinstance(audio, str):
            audio = {'url': audio}
        if audio and (not isinstance(audio, dict) or 'url' not in audio or (not utils.check_url(audio['url']))):
            logger.error('Invalid type for `audio`. Expected `dict` with `url` key, got `%s`', type(audio))
            return False
        if location and (not isinstance(location, (list, tuple)) or len(location) != 2 or (not all((isinstance(item, float) for item in location)))):
            logger.error('Invalid type for `location`. Expected `list` or `tuple` with 2 `float` items, got `%s`', type(location))
            return False
        if [photo is not None, gif is not None, file is not None, video is not None, audio is not None, location is not None].count(True) > 1:
        ...
        if not reply_markup and (not ttl):
            ...
            reply_markup = self._validate_markup({'text': '\xad', 'data': '\xad'})
        else:
            base_reply_markup = Placeholder()
        if not any((any(('callback' in button or 'input' in button for button in row)) for row in reply_markup)) and (not ttl):
            logger.debug("Patching form ttl to 10 minutes, because it doesn't contain any buttons")
            ttl = 10 * 60
        self._units[unit_id] = {'type': 'form', 'text': text, 'buttons': reply_markup, 'caller': message, 'chat': None, 'message_id': None, 'top_msg_id': utils.get_topic(message), 'uid': unit_id, 'on_unload': on_unload, 'future': Event(), **({'photo': photo} if photo else {}), **({'video': video} if video else {}), **({'gif': gif} if gif else {}), **({'location': location} if location else {}), **({'audio': audio} if audio else {}), **({'location': location} if location else {}), **({'perms_map': perms_map} if perms_map else {}), **({'message': message} if isinstance(message, Message) else {}), **({'force_me': force_me} if force_me else {}), **({'disable_security': disable_security} if disable_security else {}), **({'ttl': round(time.time()) + ttl} if ttl else {}), **({'always_allow': always_allow} if always_allow else {})}
    ...
    async def _form_inline_handler(self, inline_query: InlineQuery):
        ...
        try:
            if 'photo' in form:
                ...
                await inline_query.answer([InlineQueryResultLocation(id=utils.rand(20), latitude=form['location'][0], longitude=form['location'][1], title='Xilla', reply_markup=self.generate_markup(form['uid']))], cache_time=0)
            elif 'audio' in form:
