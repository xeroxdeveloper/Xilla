...
class List(InlineUnit):
    ...
    async def list(self, message: typing.Union[Message, int], strings: typing.List[str], *, force_me: bool=False, always_allow: typing.Optional[typing.List[int]]=None, manual_security: bool=False, disable_security: bool=False, ttl: typing.Union[int, bool]=False, on_unload: typing.Optional[typing.Callable[[], typing.Any]]=None, silent: bool=False, custom_buttons: typing.Optional[XillaReplyMarkup]=None) -> typing.Union[bool, InlineMessage]:
        ...
        if not isinstance(strings, list) or not strings:
            logger.error('Invalid type for `strings`. Expected `list` with at least one element, got `%s`', type(strings))
            return False
        if len(strings) > 50:
        ...
        self._units[unit_id] = {'type': 'list', 'caller': message, 'chat': None, 'message_id': None, 'top_msg_id': utils.get_topic(message), 'uid': unit_id, 'current_index': 0, 'strings': strings, 'future': asyncio.Event(), **({'ttl': round(time.time()) + ttl} if ttl else {}), **({'force_me': force_me} if force_me else {}), **({'disable_security': disable_security} if disable_security else {}), **({'on_unload': on_unload} if callable(on_unload) else {}), **({'always_allow': always_allow} if always_allow else {}), **({'perms_map': perms_map} if perms_map else {}), **({'message': message} if isinstance(message, Message) else {}), **({'custom_buttons': custom_buttons} if custom_buttons else {})}
    ...
    async def _list_page(self, call: CallbackQuery, page: typing.Union[int, str], unit_id: str=None):
        if page == 'close':
            await self._delete_unit_message(call, unit_id=unit_id)
            return
        if self._units[unit_id]['current_index'] < 0 or page >= len(self._units[unit_id]['strings']):
            await call.answer("Can't go to this page", show_alert=True)
            return
        self._units[unit_id]['current_index'] = page
        try:
            await self.bot.edit_message_text(inline_message_id=call.inline_message_id, text=self.sanitise_text(self._units[unit_id]['strings'][self._units[unit_id]['current_index']]), reply_markup=self._list_markup(unit_id))
            await call.answer()
    ...
    def _list_markup(self, unit_id: str) -> InlineKeyboardMarkup:
        ...
        return self.generate_markup(self._units[unit_id].get('custom_buttons', []) + self.build_pagination(callback=callback, total_pages=len(self._units[unit_id]['strings']), unit_id=unit_id) + [[{'text': '🔻 Close', 'callback': callback, 'args': ('close',)}]])
