...
class InlineMessage:
    ...
    async def edit(self, *args, **kwargs) -> 'InlineMessage':
        ...
        return await self.inline_manager._edit_unit(*args, unit_id=self.unit_id, inline_message_id=self.inline_message_id, **kwargs)

    async def delete(self) -> bool:
        return await self.inline_manager._delete_unit_message(self, unit_id=self.unit_id)
...
class BotInlineMessage:
    ...
    async def edit(self, *args, **kwargs) -> 'BotMessage':
        ...
        if 'chat_id' in kwargs:
            kwargs.pop('chat_id')
        return await self.inline_manager._edit_unit(*args, unit_id=self.unit_id, chat_id=self.chat_id, message_id=self.message_id, **kwargs)

    async def delete(self) -> bool:
        return await self.inline_manager._delete_unit_message(self, unit_id=self.unit_id, chat_id=self.chat_id, message_id=self.message_id)

    async def unload(self, *args, **kwargs) -> bool:
        if 'unit_id' in kwargs:
            kwargs.pop('unit_id')
        return await self.inline_manager._unload_unit(*args, unit_id=self.unit_id, **kwargs)
...
class InlineQuery(AiogramInlineQuery):
    ...
    async def e403(self):
        await self.answer(self._get_res('🚫 403', 'You have no permissions to access this result', 'https://img.icons8.com/external-wanicon-flat-wanicon/344/external-forbidden-new-normal-wanicon-flat-wanicon.png'), cache_time=0)

    async def e404(self):
        await self.answer(self._get_res('🚫 404', 'No results found', 'https://img.icons8.com/external-justicon-flat-justicon/344/external-404-error-responsive-web-design-justicon-flat-justicon.png'), cache_time=0)

    async def e426(self):
        await self.answer(self._get_res('🚫 426', 'You need to update Xilla before sending this request', 'https://img.icons8.com/fluency/344/approve-and-update.png'), cache_time=0)

    async def e500(self):
        await self.answer(self._get_res('🚫 500', 'Internal userbot error while processing request. More info in logs', 'https://img.icons8.com/external-vitaliy-gorbachev-flat-vitaly-gorbachev/344/external-error-internet-security-vitaliy-gorbachev-flat-vitaly-gorbachev.png'), cache_time=0)