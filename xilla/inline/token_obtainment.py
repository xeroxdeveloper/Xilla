...
class TokenObtainment(InlineUnit):
    ...
    async def _assert_token(self, create_new_if_needed: bool=True, revoke_token: bool=False) -> bool:
        if self._token:
            return True
        ...
        async with self._client.conversation('@BotFather', exclusive=False) as conv:
            ...
            for row in r.reply_markup.rows:
                for button in row.buttons:
                    if self._db.get('xilla.inline', 'custom_bot', False) and self._db.get('xilla.inline', 'custom_bot', False) != button.text.strip('@'):
                        continue
                    if not self._db.get('xilla.inline', 'custom_bot', False) and (not re.search('@xilla_[0-9a-zA-Z]{6}_bot', button.text)):
                        continue
                    await fw_protect()
                    ...
                    await m.delete()
                    await r.delete()
                    for msg in ['/setinline', button.text, 'user@xilla:~$', '/setinlinefeedback', button.text, 'Enabled', '/setuserpic', button.text]:
                        await fw_protect()
                        m = await conv.send_message(msg)
                        r = await conv.get_response()
                        logger.debug('>> %s', m.raw_text)
