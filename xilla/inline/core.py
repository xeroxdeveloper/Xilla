...
from xillatl.errors.rpcerrorlist import InputUserDeactivatedError, YouBlockedUserError
...
class InlineManager(Utils, Events, TokenObtainment, Form, Gallery, QueryGallery, List, BotPM):

    def __init__(self, client: CustomTelegramClient, db: Database, allmodules: 'Modules'):
        self._client = client
        self._db = db
    ...
    async def register_manager(self, after_break: bool=False, ignore_token_checks: bool=False):
    ...
    async def _invoke_unit(self, unit_id: str, message: Message) -> Message:
        ...
        _, pending = await asyncio.wait([result_getter_task, event_poller_task], return_when=asyncio.FIRST_COMPLETED)
        for task in pending:
            task.cancel()
        ...
        return await q[0].click(utils.get_chat_id(message) if isinstance(message, Message) else message, reply_to=message.reply_to_msg_id if isinstance(message, Message) else None)