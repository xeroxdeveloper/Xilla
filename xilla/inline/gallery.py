...
class Gallery(InlineUnit):
    ...
    async def gallery(self, message: typing.Union[Message, int], next_handler: typing.Union[callable, typing.List[str]], caption: typing.Union[typing.List[str], str, callable]='', *, custom_buttons: typing.Optional[XillaReplyMarkup]=None, force_me: bool=False, always_allow: typing.Optional[typing.List[int]]=None, manual_security: bool=False, disable_security: bool=False, ttl: typing.Union[int, bool]=False, on_unload: typing.Optional[callable]=None, preload: typing.Union[bool, int]=False, gif: bool=False, silent: bool=False, _reattempt: bool=False) -> typing.Union[bool, InlineMessage]:
        with contextlib.suppress(AttributeError):
            _xilla_client_id_logging_tag = copy.copy(self._client.tg_id)
        custom_buttons = self._validate_markup(custom_buttons)
        if not (isinstance(caption, str) or (isinstance(caption, list) and all((isinstance(item, str) for item in caption)))) and (not callable(caption)):
            logger.error('Invalid type for `caption`. Expected `str` or `list` or `callable`, got `%s`', type(caption))
            return False
        if isinstance(caption, list):
            caption = ListGalleryHelper(caption)
        if not isinstance(manual_security, bool):
            logger.error('Invalid type for `manual_security`. Expected `bool`, got `%s`', type(manual_security))
            return False
        if not isinstance(silent, bool):
            logger.error('Invalid type for `silent`. Expected `bool`, got `%s`', type(silent))
            return False
        if not isinstance(disable_security, bool):
            logger.error('Invalid type for `disable_security`. Expected `bool`, got `%s`', type(disable_security))
            return False
        if not isinstance(message, (Message, int)):
            logger.error('Invalid type for `message`. Expected `Message` or `int`, got `%s`', type(message))
            return False
        if not isinstance(force_me, bool):
            logger.error('Invalid type for `force_me`. Expected `bool`, got `%s`', type(force_me))
            return False
        ...
        if not isinstance(preload, (bool, int)) or (isinstance(preload, bool) and preload):
            logger.error('Invalid type for `preload`. Expected `int` or `False`, got `%s`', type(preload))
            return False
        if always_allow and (not isinstance(always_allow, list)):
            logger.error('Invalid type for `always_allow`. Expected `list`, got `%s`', type(always_allow))
            return False
        if not always_allow:
            always_allow = []
        if not isinstance(ttl, int) and ttl:
            logger.error('Invalid type for `ttl`. Expected `int` or `False`, got `%s`', type(ttl))
            return False
        if isinstance(next_handler, list):
            if all((isinstance(i, str) for i in next_handler)):
                next_handler = ListGalleryHelper(next_handler)
            else:
                logger.error('Invalid type for `next_handler`. Expected `callable` or `list` of `str`, got `%s`', type(next_handler))
                return False
        ...
        async def answer(msg: str):
            ...
            if isinstance(message, Message):
                await (message.edit if message.out else message.respond)(msg, **{} if message.out else {'reply_to': utils.get_topic(message)})
            else:
        ...
        try:
            ...
            if _reattempt:
                ...
                del self._units[unit_id]
                await answer(self.translator.getkey('inline.invoke_failed_logs').format(utils.escape_html('\n'.join(traceback.format_exc().splitlines()[1:]))) if self._db.get(main.__name__, 'inlinelogs', True) else self.translator.getkey('inline.invoke_failed'))
                return False
    ...
    async def _call_photo(self, callback: typing.Union[typing.Callable[[], typing.Awaitable[str]], typing.Callable[[], str], typing.List[str]]) -> typing.Union[str, bool]:
        if isinstance(callback, str):
            ...
            logger.error('Invalid type for `next_handler`. Expected `str`, `list` or `callable`, got %s', type(callback))
            return False
        if not isinstance(photo_url, (str, list)):
            logger.error('Got invalid result from `next_handler`. Expected `str` or `list`, got %s', type(photo_url))
            return False
        return photo_url
    ...
    async def _load_gallery_photos(self, unit_id: str):
        ...
        self._units[unit_id]['photos'] += [photo_url] if isinstance(photo_url, str) else photo_url
        unit = self._units[unit_id]
        if unit.get('preload', False) and len(unit['photos']) - unit['current_index'] < unit.get('preload', False):
            asyncio.ensure_future(self._load_gallery_photos(unit_id))

    async def _gallery_slideshow_loop(self, call: CallbackQuery, unit_id: typing.Optional[str]=None):
        while True:
            await asyncio.sleep(7)
            unit = self._units[unit_id]
            if unit_id not in self._units or not unit.get('slideshow', False):
                return
            if unit['current_index'] + 1 >= len(unit['photos']) and isinstance(unit['next_handler'], ListGalleryHelper):
                del self._units[unit_id]['slideshow']
                self._units[unit_id]['current_index'] -= 1
            await self._gallery_page(call, self._units[unit_id]['current_index'] + 1, unit_id=unit_id)

    async def _gallery_slideshow(self, call: CallbackQuery, unit_id: typing.Optional[str]=None):
        if not self._units[unit_id].get('slideshow', False):
            self._units[unit_id]['slideshow'] = True
            await self.bot.edit_message_reply_markup(inline_message_id=call.inline_message_id, reply_markup=self._gallery_markup(unit_id))
    ...
    async def _gallery_back(self, call: CallbackQuery, unit_id: typing.Optional[str]=None):
        ...
        if self._units[unit_id]['current_index'] < 0:
            ...
            return
        try:
            await self.bot.edit_message_media(inline_message_id=call.inline_message_id, media=self._get_current_media(unit_id), reply_markup=self._gallery_markup(unit_id))
        except RetryAfter as e:
            await call.answer(f'Got FloodWait. Wait for {e.timeout} seconds', show_alert=True)
        except Exception:
            logger.exception('Exception while trying to edit media')
            await call.answer('Error occurred', show_alert=True)
            return

    def _get_current_media(self, unit_id: str) -> typing.Union[InputMediaPhoto, InputMediaAnimation]:
        ...
        if self._units[unit_id].get('gif', False) or ext in {'.gif', '.mp4'}:
            return InputMediaAnimation(media=media, caption=self._get_caption(unit_id, index=self._units[unit_id]['current_index']), parse_mode='HTML')
        return InputMediaPhoto(media=media, caption=self._get_caption(unit_id, index=self._units[unit_id]['current_index']), parse_mode='HTML')

    async def _gallery_page(self, call: CallbackQuery, page: typing.Union[int, str], unit_id: typing.Optional[str]=None):
        if page == 'slideshow':
        ...
        if page < 0:
            await call.answer('No way back')
            return
        if page > len(self._units[unit_id]['photos']) - 1 and isinstance(self._units[unit_id]['next_handler'], ListGalleryHelper):
        ...
        if not isinstance(self._units[unit_id]['next_handler'], ListGalleryHelper):
            ...
            if self._units[unit_id]['current_index'] >= len(self._units[unit_id]['photos']):
                ...
                return
            if len(self._units[unit_id]['photos']) - self._units[unit_id]['current_index'] < self._units[unit_id].get('preload', 0) // 2:
    ...
    def _get_next_photo(self, unit_id: str) -> str:
        try:
            ...
            logger.error('Got IndexError in `_get_next_photo`. %s / %s', self._units[unit_id]['current_index'], len(self._units[unit_id]['photos']))
    ...
    def _get_caption(self, unit_id: str, index: int=0) -> str:
        ...
        return caption if isinstance(caption, str) else caption() if callable(caption) else ''

    def _gallery_markup(self, unit_id: str) -> InlineKeyboardMarkup:
        callback = functools.partial(self._gallery_page, unit_id=unit_id)
        unit = self._units[unit_id]
        return self.generate_markup(unit.get('custom_buttons', []) + self.build_pagination(unit_id=unit_id, callback=callback, total_pages=len(unit['photos'])) + [[*([{'text': '⏪', 'callback': callback, 'args': (unit['current_index'] - 1,)}] if unit['current_index'] > 0 else []), *([{'text': '🛑' if unit.get('slideshow', False) else '⏱', 'callback': callback, 'args': ('slideshow',)}] if unit['current_index'] < len(unit['photos']) - 1 or not isinstance(unit['next_handler'], ListGalleryHelper) else []), *([{'text': '⏩', 'callback': callback, 'args': (unit['current_index'] + 1,)}] if unit['current_index'] < len(unit['photos']) - 1 or not isinstance(unit['next_handler'], ListGalleryHelper) else [])]] + [[{'text': '🔻 Close', 'callback': callback, 'args': ('close',)}]])

    async def _gallery_inline_handler(self, inline_query: InlineQuery):
        for unit in self._units.copy().values():
            if inline_query.from_user.id == self._me and inline_query.query == unit['uid'] and (unit['type'] == 'gallery'):
                try:
                    try:
                        ...
                        ext = None
                    args = {'thumb_url': 'https://img.icons8.com/fluency/344/loading.png', 'caption': self._get_caption(unit['uid'], index=0), 'parse_mode': 'HTML', 'reply_markup': self._gallery_markup(unit['uid']), 'id': utils.rand(20), 'title': 'Processing inline gallery'}
                    if unit.get('gif', False) or ext in {'.gif', '.mp4'}:
                        await inline_query.answer([InlineQueryResultGif(gif_url=unit['photo_url'], **args)])
                        return
                    await inline_query.answer([InlineQueryResultPhoto(photo_url=unit['photo_url'], **args)], cache_time=0)
