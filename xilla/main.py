...
with contextlib.suppress(Exception):
    ...
    if 'microsoft-standard' in uname().release:
        IS_WSL = True
LATIN_MOCK = ['Amor', 'Arbor', 'Astra', 'Aurum', 'Bellum', 'Caelum', 'Calor', 'Candor', 'Carpe', 'Celer', 'Certo', 'Cibus', 'Civis', 'Clemens', 'Coetus', 'Cogito', 'Conexus', 'Consilium', 'Cresco', 'Cura', 'Cursus', 'Decus', 'Deus', 'Dies', 'Digitus', 'Discipulus', 'Dominus', 'Donum', 'Dulcis', 'Durus', 'Elementum', 'Emendo', 'Ensis', 'Equus', 'Espero', 'Fidelis', 'Fides', 'Finis', 'Flamma', 'Flos', 'Fortis', 'Frater', 'Fuga', 'Fulgeo', 'Genius', 'Gloria', 'Gratia', 'Gravis', 'Habitus', 'Honor', 'Hora', 'Ignis', 'Imago', 'Imperium', 'Inceptum', 'Infinitus', 'Ingenium', 'Initium', 'Intra', 'Iunctus', 'Iustitia', 'Labor', 'Laurus', 'Lectus', 'Legio', 'Liberi', 'Libertas', 'Lumen', 'Lux', 'Magister', 'Magnus', 'Manus', 'Memoria', 'Mens', 'Mors', 'Mundo', 'Natura', 'Nexus', 'Nobilis', 'Nomen', 'Novus', 'Nox', 'Oculus', 'Omnis', 'Opus', 'Orbis', 'Ordo', 'Os', 'Pax', 'Perpetuus', 'Persona', 'Petra', 'Pietas', 'Pons', 'Populus', 'Potentia', 'Primus', 'Proelium', 'Pulcher', 'Purus', 'Quaero', 'Quies', 'Ratio', 'Regnum', 'Sanguis', 'Sapientia', 'Sensus', 'Serenus', 'Sermo', 'Signum', 'Sol', 'Solus', 'Sors', 'Spes', 'Spiritus', 'Stella', 'Summus', 'Teneo', 'Terra', 'Tigris', 'Trans', 'Tribuo', 'Tristis', 'Ultimus', 'Unitas', 'Universus', 'Uterque', 'Valde', 'Vates', 'Veritas', 'Verus', 'Vester', 'Via', 'Victoria', 'Vita', 'Vox', 'Vultus', 'Zephyrus']
...
def parse_arguments() -> dict:
    ...
    parser.add_argument('--qr-login', dest='qr_login', action='store_true', help='Use QR code login instead of phone number (will only work if scanned from another device)')
    parser.add_argument('--data-root', dest='data_root', default='', help='Root path to store session files in')
    parser.add_argument('--no-auth', dest='no_auth', action='store_true', help='Disable authentication and API token input, exitting if needed')
    parser.add_argument('--proxy-host', dest='proxy_host', action='store', help='MTProto proxy host, without port')
    parser.add_argument('--proxy-port', dest='proxy_port', action='store', type=int, help='MTProto proxy port')
    parser.add_argument('--proxy-secret', dest='proxy_secret', action='store', help='MTProto proxy secret')
    parser.add_argument('--root', dest='disable_root_check', action='store_true', help='Disable `force_insecure` warning')
    parser.add_argument('--proxy-pass', dest='proxy_pass', action='store_true', help='Open proxy pass tunnel on start (not needed on setup)')
    parser.add_argument('--no-tty', dest='tty', action='store_false', default=True, help='Do not print colorful output using ANSI escapes')
...
class Xilla:
    ...
    def _get_proxy(self):
        if self.arguments.proxy_host is not None and self.arguments.proxy_port is not None and (self.arguments.proxy_secret is not None):
            logging.debug('Using proxy: %s:%s', self.arguments.proxy_host, self.arguments.proxy_port)
            self.proxy, self.conn = ((self.arguments.proxy_host, self.arguments.proxy_port, self.arguments.proxy_secret), ConnectionTcpMTProxyRandomizedIntermediate)
            return
        self.proxy, self.conn = (None, ConnectionTcpFull)

    def _read_sessions(self):
        self.sessions = []
        self.sessions += [SQLiteSession(os.path.join(BASE_DIR, session.rsplit('.session', maxsplit=1)[0])) for session in filter(lambda f: f.startswith('xilla-') and f.endswith('.session'), os.listdir(BASE_DIR))]

    def _get_api_token(self):
        api_token_type = collections.namedtuple('api_token', ('ID', 'HASH'))
        try:
            if not get_config_key('api_id'):
                api_id, api_hash = (line.strip() for line in (Path(BASE_DIR) / 'api_token.txt').read_text().splitlines())
    ...
    async def _initial_setup(self) -> bool:
        ...
        if not self.web:
            ...
            def print_qr():
                ...
                print('\x1b[2J\x1b[3;1f')
                qr.print_ascii(invert=True)
                print('\x1b[0;96mScan the QR code above to log in.\x1b[0m')
                print('\x1b[0;96mPress Ctrl+C to cancel.\x1b[0m')
            ...
            if qr_logined:
                ...
                while True:
                    _2fa = getpass(f'\x1b[0;96mEnter 2FA password ({password.hint}): \x1b[0m' if IS_TERMUX or self.arguments.tty else f'Enter 2FA password ({password.hint}): ')
                    try:
                        await client._on_login((await client(CheckPasswordRequest(compute_check(password, _2fa.strip())))).user)
                    except PasswordHashInvalidError:
                        print('\x1b[0;91mInvalid 2FA password!\x1b[0m')
                    except FloodWaitError as e:
                        seconds, minutes, hours = (e.seconds % 3600 % 60, e.seconds % 3600 // 60, e.seconds // 3600)
                        seconds, minutes, hours = (f'{seconds} second(-s)', f'{minutes} minute(-s) ' if minutes else '', f'{hours} hour(-s) ' if hours else '')
                        print(f'\x1b[0;91mYou got FloodWait error! Please wait {hours}{minutes}{seconds}\x1b[0m')
                        return False
                    else:
                        break
            print_banner('success.txt')
            print('\x1b[0;92mLogged in successfully!\x1b[0m')
            await self.save_client_session(client)
            self.clients += [client]
            return True
    ...
    async def _init_clients(self) -> bool:
        for session in self.sessions.copy():
            try:
                client = CustomTelegramClient(session, self.api_token.ID, self.api_token.HASH, connection=self.conn, proxy=self.proxy, connection_retries=None, device_model=get_app_name(), system_version='Windows 10', app_version='.'.join(map(str, __version__)) + ' x64', lang_code='en', system_lang_code='en-US')
                await client.start(phone=raise_auth if self.web else lambda: input('\x1b[0;96mEnter phone: \x1b[0m' if IS_TERMUX or self.arguments.tty else 'Enter phone: '))
                client.phone = 'never gonna give you up'
                self.clients += [client]
            except sqlite3.OperationalError:
                logging.error("Check that this is the only instance running. If that doesn't help, delete the file '%s'", session.filename)
                continue
                ...
                logging.error("Phone number is incorrect. Use international format (+XX...) and don't put spaces in it.")
                self.sessions.remove(session)
            except InteractiveAuthRequired:
                logging.error('Session %s was terminated and re-auth is required', session.filename)
                self.sessions.remove(session)
    ...
    async def _add_dispatcher(self, client: CustomTelegramClient, modules: loader.Modules, db: database.Database):
        ...
        client.add_event_handler(dispatcher.handle_command, events.NewMessage(forwards=False))
    ...
    async def _main(self):
        ...
        if (not self.clients and (not self.sessions) or not await self._init_clients()) and (not await self._initial_setup()):
            return
