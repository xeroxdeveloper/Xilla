...
from xillatl.network.connection import (
    ConnectionTcpFull,
...
class Xilla:
    ...
    def _read_sessions(self):
        ...
        self.sessions += [
            ...
            for session in filter(
                lambda f: f.startswith("xilla-") and f.endswith(".session"),
                os.listdir(BASE_DIR),
            )
        ]
    ...
    async def _badge(self, client: CustomTelegramClient):
        ...
        try:
            ...
            if not self.omit_log:
                ...
                self.omit_log = True
            await client.xilla_inline.bot.send_animation(
                logging.getLogger().handlers[0].get_logid_by_client(client.tg_id),
                "https://github.com/xeroxdeveloper/assets/raw/master/xilla_banner.mp4",
                caption=(
                    "🌘 <b>Xilla {} started!</b>\n\n🌳 <b>GitHub commit SHA: <a"
                    ' href="https://github.com/xeroxdeveloper/Xilla/commit/{}">{}</a></b>\n✊'
                    " <b>Update status: {}</b>\n<b>{}</b>".format(
                        ".".join(list(map(str, list(__version__)))),
                        build,
                        build[:7],
    ...
    async def _main(self):
        ...
        # --- XILLA SUPERVISOR (Watchdog) ---
        # Ensures 99.99% uptime, better than Heroku. 
        # If the bot crashes, it restarts the task instantly instead of killing the process.
