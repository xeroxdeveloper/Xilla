...
class DragonDb:
    ...
    def get(self, module: str, variable: str, default: typing.Optional[typing.Any]=None) -> JSONSerializable:
...
class Notifier:
    ...
    @property
    def commands(self):
        return {key.split()[0]: (key.split()[1] + ' - ' if len(key.split()) > 1 else '') + value for key, value in self.cache[self.modname].items()}
...
class DragonMisc:
    ...
    def __init__(self, client: CustomTelegramClient):
        ...
        self.gitrepo = git.Repo(path=os.path.abspath(os.path.join(os.path.dirname(version.__file__), '..')))
...
class DragonScripts:
    ...
    def format_exc(e: Exception, hint: str=None):
        ...
        if isinstance(e, errors.RPCError):
            return f'<b>Telegram API error!</b>\n<code>[{e.CODE} {e.ID or e.NAME}] - {e.MESSAGE}</code>'
    ...
    def format_module_help(self, module_name: str):
        ...
        help_text = f'{DRAGON_EMOJI} <b>Help for </b><code>{module_name}</code>\n\n<b>Usage:</b>\n'
        for command, desc in commands.items():
            cmd = command.split(maxsplit=1)
            args = ' <code>' + cmd[1] + '</code>' if len(cmd) > 1 else ''
            help_text += f'<code>{self.misc.prefix}{cmd[0]}</code>{args} — <i>{desc}</i>\n'
        return help_text
    ...
    def format_small_module_help(self, module_name: str):
        ...
        help_text += f'\n<b>Get full usage:</b> <code>{self.misc.prefix}help {module_name}</code>'
        return help_text

    def import_library(self, library_name: str, package_name: typing.Optional[str]=None):
        ...
        try:
            ...
            completed = subprocess.run([sys.executable, '-m', 'pip', 'install', '--upgrade', '-q', '--disable-pip-version-check', '--no-warn-script-location', *(['--user'] if 'PIP_TARGET' not in os.environ and 'VIRTUAL_ENV' not in os.environ else []), package_name], check=False)
            if completed.returncode != 0:
                raise RuntimeError(f'Failed to install library {package_name} (pip exited with code {completed.returncode})')
            return importlib.import_module(library_name)

    @staticmethod
    def resize_image(input_img: typing.Union[bytes, io.BytesIO], output: typing.Optional[io.BytesIO]=None, img_type: str='PNG') -> io.BytesIO:
